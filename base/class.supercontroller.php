...
class SuperController extends SuperUtility
{
    ...
    /** @var string what kind of access should be served */
    protected $access = "";
    ...
    public function __construct($data, $model)
    {
        $this->data = $data;
        $this->model = $model;
        $this->access = strtolower(self::getOrFallBack($data, "access", "view"));
    }
    ...
    function __destruct()
    {
        ...
        if ($this->request != null) {
            $outputArray = array_merge($outputArray, array("action" => $this->request));
        }
    ...
    public function start($template = null)
    {
        ...
        if (isset($this->data['request']) && method_exists($this, $this->data['request'])) {

            $reflect = new \ReflectionMethod($this, $this->data['request']);

            if ($reflect->isProtected()) {
                $reflect->setAccessible(true);

                $this->request = $reflect->getName();
